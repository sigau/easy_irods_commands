...
def to_irods_and_beyond():
    ...
    with iRODSSession(**easicmd.irods_config) as session:
        if type_object == "-d":
            ...
            easicmd.copy_folder_to_irods(local_object,local_object,irods_path,session,easicmd.list_of_icollection)
...
def DOWNLOAD(itype):
    ...
    with iRODSSession(**easicmd.irods_config) as session:
        if itype == "-C" :
            easicmd.copy_irod_to_folder(session,irods_path.replace("//", "/"),local_path.replace("//", "/"), recursive=True)
        else :
            session.data_objects.get(irods_path_file.replace("//", "/"),local_path.replace("//", "/"))    
