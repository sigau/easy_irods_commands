    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "push":
            ...
            if len(sys.argv) > 3:
                ...
                print(
                    "you need to give at least the type of data you want to download from irods (-f or -C ) \nyou can also give the path to which you want to download the object\n command should look like easicmd.py pull -f/-C local/path/to/download"
                )
            ...
            get_irods_collection()
            irods_path = prompt(
                "ifolder (empty = /zone/home/user ): ", completer=icol_completer
            )
            IDUSH(irods_path)
...
def help():
    ...
    print(
        "\tadd_path\t: add additional path to your irods path autocompletion (e.g : not my home but a common folder for a project)(stock in a file for later)"
    )
    print(
...
def get_irods_password():
    ...
    ## password wasn't save we can just ask it
        ...
        if caller == "__main__":
            ...
            answer = input(
                f"Do you want to save it for later (the password will be encrypt and stock in {irods_password_path} [Y/N] : "
            )
...
def update_irods_collection(action, path):
    ...
    if action == "add":
        ...
        list_of_icollection.sort()
        list_of_icollection = list(
            dict.fromkeys(list_of_icollection)
        )  ## no double in my list please
...
def list_ifile(ifolder):
    ...
    with iRODSSession(**irods_config) as session:
        results = session.query(Collection.name, DataObject.name).filter(
            Criterion("=", Collection.name, ifolder)
        )
...
def local_collection():
    ...
    dossiers_non_caches = [
        d for d in lister_dossiers_recursivement(os.path.expanduser("~"))
    ]  ## from the user home
...
def lister_dossiers_recursivement(chemin):
    ...
    try:
        for nom in os.listdir(chemin):
            ...
            if os.path.isdir(chemin_complet) and not (
...
def list_local_file(folder):
    ...
    local_file = [
        f
        for f in os.listdir(os.path.expanduser(folder))
        if os.path.isfile(os.path.join(os.path.expanduser(folder), f))
    ]
    local_file.sort()
...
def call_iobject(type_iobject):
    ...
    if type_iobject == "-f":
        list_ifile(iobject)
        iobject = (
            iobject + "/" + prompt("irods file (tap tab) :", completer=ifile_completer)
        )
...
def copy_folder_to_irods(
    racine, local_folder_path, irods_destination_path, session, list_of_icollection
):
    for root, dirs, files in os.walk(local_folder_path):
        ## Create the iRODS collection for the local folder if it does not exist
        relative_path = (
            racine.split("/")[-1] + "/" + os.path.relpath(root, local_folder_path)
        )
        ...
        for file in files:
            ...
            session.data_objects.put(
                local_file_path, irods_file_path, num_threads=nb_threads
            )
    ...
    for dir in dirs:
        ...
        copy_folder_to_irods(
            racine,
            local_subfolder_path,
            irods_subfolder_path,
            session,
            list_of_icollections,
        )
    ...
    if session.data_objects.exists(irods_path):
        ...
        if recursive:
            ...
            for file_object in coll.data_objects:
                copy_irod_to_folder(
                    session,
                    os.path.join(irods_path, file_object.path),
                    local_path,
                    True,
                )
...
def PUSH(local_object):
    ...
    with iRODSSession(**irods_config) as session:
        if is_file:
            ...
            copy_folder_to_irods(
                local_object, local_object, irods_path, session, list_of_icollection
            )
    ...
    if (
        local_object[-1] == "/"
    ):  # if a folder is given with the / in the end we remove it
        local_object = local_object[:-1]
    if (
        "/" in local_object
    ):  ##when path (path/to/object) given we keep only the end as in irods
        local_object = (local_object.split("/"))[
            -1
        ]  ## the new path will be irods_path/object
    ...
    with iRODSSession(**irods_config) as session:
        if type_iobject == "-f":
            ...
            if "*" in iobject or "%" in iobject:
                print(
                    "not working yet --> download one by one or download the collection"
                )
            else:
...
def ADD_META(iobject):
    ...
    while attribut != "":
        attribut_completer = WordCompleter(
            list_key
        )  ##if you don't want to have autocompletion on this command comment this
        attribut = prompt(
            "attribut (empty to stop) : ", completer=attribut_completer
        )  ##AND THIS
...
def SEARCH_BY_META(type_iobject=None):
    ...
    while liaison != "":
        ...
        if not type_iobject:
            build_cmd_C = (
                build_cmd_C
                + f"CollectionMeta.name == '{qu_attribute}' and CollectionMeta.value == '{qu_value}' {liaison} "
            )
            build_cmd_F = (
                build_cmd_F
                + f"DataObjectMeta.name == '{qu_attribute}' and DataObjectMeta.value == '{qu_value}' {liaison} "
            )
        elif type_iobject == "-C":
            build_cmd_C = (
                build_cmd_C
...
def SEARCH_BY_NAME(tofind, type_iobject=None):
    ...
    with iRODSSession(**irods_config) as session:
        if not type_iobject:
            ...
            # list collection
            results_C = session.query(Collection.name).filter(
                Criterion("like", Collection.name, tofind)
            )
            ## list file
            results_F = session.query(Collection.name, DataObject.name).filter(
                Criterion("like", DataObject.name, tofind)
            )
...
def ADD_ADDITIONAL_PATH():
    ...
    if new_path == "":
        ...
        print("irods list of path updated")
        print(
            f"updating the irods collection pickle with new collection from {new_path}"
        )
...
def RM_ADDITIONAL_PATH():
    ...
    if os.path.isfile(new_path_file):
        ...
        to_remove = prompt(
            "which to remove (empty to stop):", completer=list_path_completer
        )
        if to_remove == "":
            sys.exit()
...
def EDITIONARY():
    ...
    attribute = prompt(
        "choose a attribute or create a new one : ", completer=attribute_completer
    )
